...
function gebcount(){
	...
	<br />
	<center>
		<form action="configseite.php" id="configform" method="POST"> {* onSubmit="{literal}if(typeof submitTutorial == 'function') {return submitTutorial();} else {return true;}{/literal}" *}
			...
			<input  TYPE="hidden" VALUE="{$status.id}" NAME="id"><br />
			<center>Achtung! Nach beenden der Konfigurationsphase kann die gewählte Konfiguration nur durch
			vollständiges zurücksetzen/resetten deines Konzerns unter Optionen geändert werden.</center>
			<input type="submit" value="Konfiguration beenden">
		</form>
		<a name="isPro" class="linkAuftableInner" href="#isPro" onClick="javascript:document.getElementById('isPro').style.display = 'table-row';">Erweitern (für Fortgeschrittene)</a>
		<table width="320" cellpadding="0" cellspacing="0" border="0" class="tableOutline">
			<tr id="isPro" style="display:none;">
				<td>
					<table border="0" cellspacing="1" width="320" cellpadding="2">
						<tr class="tableHead">
						...
						<tr class="tableInner1">
							...
							<td align="center">
								<input type="text" style="width: 5em;" dojoType="dijit.form.NumberTextBox" id = "land" name= "land" value="{$LAND_MIN}" onKeyUp = "gebcount()" constraints="{literal}{{/literal}min:{$LAND_MIN},max:{$LAND_MAX},places:0{literal}}{/literal}" promptMessage= "Nur Zahlen zwischen {$LAND_MIN} und {$LAND_MAX} eingeben!" required= "true" invalidMessage= "Ungültiger Wert."> ha
							</td>
						</tr>
						<tr class="tableInner1">
							<td colspan="2">
								Dieses Eingabefeld dient nur zur Simulation, wie die Produktion und die Boni aussehen nachdem das erste mal Land gekauft wurde.
							</td>
						</tr>
					</table>
				</td>

...
foreach ($rids_total as $ky => $vl) {
	...
	$ausgabe .= "<tr><td".($syndikate[$ky][synd_type] != "normal" ? " bgcolor=yellow":"").">".$syndikate[$ky][synd_type]."</td><td><a href=index.php?action=checksyndikatsgroessen&synnummer=$ky>".$syndikate[$ky][name]."</a></td><td>#$ky</td><td".(($rids[$ky][tl] > 0 and $syndikate[$ky][synd_type] != "normal") ? " bgcolor=red":"").">".($rids[$ky][tl] + $rids_isnoob[$ky][tl] > MAX_USERS_A_SYNDICATE ? "<font color=blue size=$size><b>".$rids[$ky][tl]."</b></font>":$rids[$ky][tl])."</td><td".(($rids_isnoob[$ky][tl] > 0 and $syndikate[$ky][synd_type] == "normal") ? " bgcolor=red":"").">".($rids[$ky][tl] + $rids_isnoob[$ky][tl] > MAX_USERS_A_SYNDICATE ? "<font color=blue size=$size><b>".$rids_isnoob[$ky][tl]."</b></font>":$rids_isnoob[$ky][tl])."</td></tr>";
	$count++;
	$sum1 += $rids[$ky][tl];
...
if ($synnummer) {
	...
	if ($pid && ($isadmin or ($players_stats[$pid] && !$players[$pid] || !$players_stats[$pid] && $players[$pid]))) {
		...
		if ($isadmin) $ausgabe .= "<tr><td></td><td><a href=index.php?action=checksyndikatsgroessen&synnummer=$synnummer&pid=$pid&deletestats=1>löschen</a></td><td><!--<a href=index.php?action=checksyndikatsgroessen&synnummer=$synnummer&pid=$pid&deletestatus=1>löschen</a>--></td></tr>";
		...
		if (($deletestats xor $deletestatus) and $isadmin) {
			unset ($ausgabe);
			if (!$continue) {
				$ausgabe = "SICHERHEITSABFRAGE, Weiter?<br><br><a href=index.php?action=checksyndikatsgroessen&synnummer=$synnummer&pid=$pid&".($deletestats ? "deletestats":"deletestatus")."=1&continue=1>JA</a>";
			}
			else {
				if ($deletestats) {

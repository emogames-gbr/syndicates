...
elseif ($userid && $killfinal) {
    if (!$reason) {echo "bitte grund angeben";}
    else {
        $konzernid = single("select konzernid from users where id = $userid");
        echo $konzernid;
        if ($konzernid) {
            if ($killfinal == "konzern") {
            }
            if ($killfinal == "user") {
                kill_den_konzern($konzernid,0,2,$userid);
                select("insert into admin_users_punished (user_id,time,reason,action) values ($userid,$time,'$reason','account gelöscht')");
            }
            if ($killfinal == "ban") {
                kill_den_konzern($konzernid,0,3,$userid);
                select("insert into admin_users_punished (user_id,time,reason,action) values ($userid,$time,'$reason','account gebannt')");
            }
            $ausgabe.="... erfolgreich gelöscht";
        }
        else {
            $ausgabe.="Konzern nicht gefunden";
        }
    }
    $ausgabe.="<br><br><a href=\"$self\">Zurück</a>";

    ...
    if (count($$value) > 1) {
        ...
        $ausgabe_temp.="<table  width=\"100%\" cellspacing=0 cellpadding=0 style=\"border:1px solid\" border=1>
                    <tr>
                        ...
                        <td width=\"100\" align=\"left\">Passwortvergleich*</td>
        ...
        foreach ($$value as $user) {
			...
			if($user){
				$emoid=single("select emogames_user_id from users where konzernid='".$user."'");
				$psid=single("select partnership from admin_ip_partners where emoid=".$emoid);
				if(!$currentPs)
					$currentPs=$psid;
				$color = (($currentPs == $psid) && ($psid!=0)) ? "#99FF99" : "#FF6633";
				
			}
			if ($nw and $status[nw] > $nw): $invalidip = 0; endif;
            $ct = date("d.m.y - H:i",$status{createtime});
            $lastloginstart= date("d.m.y - H:i:s",${$user}[lastloginstart]);
            $lastloginende= date("d.m.y - H:i:s",${$user}[lastloginende]);
            $ausgabe_temp.= "<tr bgcolor=".$color." ";
			...
			$ausgabe_temp .=">
							...
							<td>".$status{email}."</td>
							<td>".substr(hash('sha256', 'pwsaltSYN'.$status{password}),0,15)."</td>
							<td>$ct</td>
...
$ausgabe .= "Passwortvergleich: Hier wird nur ein verürzer Hashwert (sha256) des verschlüsselten Passworts angezeigt. 
	Es muss nicht zwingend das selbe Passwort am Ende benutzt worden sein. 
	Für genaueren Vergleich schaue unter \"Gleiche Passwörter\" nach. (inok1989 09.12.2012)";

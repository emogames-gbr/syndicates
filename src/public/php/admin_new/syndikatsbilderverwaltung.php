...
$self = "syndikatsbilderverwaltung.php";
$ausgabe ="";

	$si_pfad_absolut = DATA."/syndikatsimages";
	$si_pfad_relativ = WWWDATA."syndikatsimages";
	
	$disapproved_pfad_absolut = PUB."/admin/syndikatsimages_disapproved";
	$disapproved_pfad_relativ = WWWPUB."syndikatsimages_disapproved";
	
	
	
if ($mode == "single" and 1 == 2)	{ 
	...
	if ($konzernid)	{
		...
		if ($innestaction == 1)	{
				nack($konzernid);
			$ausgabe .= "<br><br><a href=$self?action=syndikatsbilderverwaltung&actn=vki&mode=$mode2>zurück</a>&nbsp;&nbsp;&nbsp; - &nbsp;&nbsp;&nbsp;<a href=$self?action=syndikatsbilderverwaltung&actn=vki&mode=single&konzernid=$konzernid>zurück zur Userübersicht</a>";
		}
		elseif (!$innestaction)	{
			if ($userdaten[image])	{
				$ausgabe .= "Bisheriges Konzernbild<br><br><img src=".$ki_pfad_relativ."/konzern_".$konzernid.".".$userdaten[image].">";
				$ablehnen_optional = "&nbsp;&nbsp;&nbsp; - &nbsp;&nbsp;&nbsp;<a href=$self?action=syndikatsbilderverwaltung&actn=vki&mode=single&konzernid=$konzernid&innestaction=1>Bild ablehnen</a>";
			}
			else { $ausgabe .= "User hat kein Konzernbild";}
			$ausgabe .= "<br><br><a href=$self?action=syndikatsbilderverwaltung&actn=vki&mode=$mode2>zurück</a>".$ablehnen_optional;
...
elseif ($mode == "showall")	{
	$images = assocs("select synd_id, name,image from syndikate where image != ''", "synd_id");
	
	$zaehler = 1;
	$spalten = 2; # Mindestens 2;
	
	$ausgabe .= "<center><a href=$self?action=syndikatsbilderverwaltung&actn=vsi>zurück</a></center>
				<br><br>
	...
	foreach ($images as $ky => $vl)	{
		...
		$ausgabe .= "$pre<td align=center valign=middle width=110 height=140><a href=$self?action=syndikatsbilderverwaltung&actn=vsi&mode=single&mode2=showall&syndikatsid=$ky><img src=$si_pfad_relativ/syndikat_".$ky.".".$vl[image]." border=0 title=\"<".$vl[name]."><".$ky."> - Syndikat ".$vl[synd_id]."\"></a></td>$after";
		$zaehler++;
	}
	$ausgabe .= "</table>
				<br><br>
				<center><a href=$self?action=syndikatsbilderverwaltung&actn=vsi>zurück</a></center>";
}
elseif ($mode == "pap" and 1 == 2)	{ # Pic After Pic
	$ausgabe .= "<center><a href=$self?action=syndikatsbilderverwaltung&actn=vsi>zurück</a></center>
				<br><br>";
	...
	$ausgabe .= "
		...
		<form action=$self method=post>
			<input type=hidden name=action value=syndikatsbilderverwaltung>
	...
	$ausgabe .= "<br><br>
					<center><a href=$self?action=syndikatsbilderverwaltung&actn=vki>zurück</a></center>";
}

else	{

	$ausgabe .= "<center><a href=$self>zurück</a></center>
				<br><br><br><br><br><br><a href=$self?action=syndikatsbilderverwaltung&actn=vsi&mode=pap>Pics After Pics (APPROVE MODE)</a><br><br><a href=$self?action=syndikatsbilderverwaltung&actn=vsi&mode=showall>Showall (FUN MODE)</a><br><br><br><br><br><br>

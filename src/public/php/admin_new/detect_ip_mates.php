...
echo "<html><head><title>Syndicates Adminpanel</title></head>
	<body><table bgcolor=white width=100% align=left><tr><td>";
...
if (!$konzernid && !$ip) {
	...
	echo "	<form action=index.php method=post><input name=action value=detect_ip_mates type=hidden>
	...
	<!--
		function info(konzernid, ip)	{
			neuesFenster =  open("detect_ip_mates.php?konzernid="+konzernid+"&ip="+ip, "Info","width=800, height=600, scrollbars=yes");
			neuesFenster.focus()
		};
	-->
	</script>
	...
	if ($konzernid) {
		move_old_sessionids_actual_to_safe_table($konzernid);
		if (!$ip) {
		  $sessions = assocs("select * from sessionids_safe where user_id = $konzernid and angelegt_bei >= ".($globals[roundstarttime]-3*86400)." order by angelegt_bei desc");
		} else if ($ip) {
		  $parts = explode(".", $ip);
		  for ($i = 0; $i <= 1; $i++) {
		    if (!preg_match("/\d+/", $parts[$i])) exit("Die ersten 2 Blöcke der IP müssen Zahlen sein. % erst ab dem 3. Block erlaubt.");
		  }
		  $sessions = assocs("select * from sessionids_safe where (ip like '$ip' or iptrue like '$ip') and angelegt_bei >= ".($globals[roundstarttime]-3*86400)." order by angelegt_bei desc");
		}
		foreach ($sessions as $vl) {
			if ($ip) {
			  $sessions_by_ips[$ip][] = $vl;
			} else {
			  $sessions_by_ips[$vl[ip]][] = $vl;
			}
			if (!in_array($vl[ip], $ips)) $ips[] = $vl[ip];
			if (!in_array($vl[ip], $ipstrue)) $ipstrue[] = $vl[iptrue];
		...
		foreach ($sessions_by_ips as $ky => $vl) {
			$total_sessions_by_ips[$ky] = assocs("select sessionid, angelegt_bei, gueltig_bis, ip, iptrue, user_id, gueltig_bis - angelegt_bei as validtime, hostname, pc_identifier, browsername from sessionids_safe where (ip like '$ky' or iptrue like '$ky') and angelegt_bei >= ".($globals[roundstarttime]-3*86400)." order by angelegt_bei desc");
		...
		foreach ($sessions_by_pcid as $ky => $vl) {
			$total_sessions_by_pcid[$ky] = assocs("select sessionid, angelegt_bei, gueltig_bis, ip, iptrue, user_id, gueltig_bis - angelegt_bei as validtime, hostname, pc_identifier, browsername from sessionids_safe where pc_identifier like '$ky' and angelegt_bei >= ".($globals[roundstarttime]-3*86400)." order by angelegt_bei desc");
			$lastused_done = 0;
	...
	if (!$ip && !$pcid && !$session_id) {
		...
		foreach ($ips as $kk=>$vl) {
			...
			// The ip link is splitted in 2 parts. The first is the one with the 4. block substitued by %, the second is the original ip
			$ip_parts = explode(".", $vl);
			$ip123 = $ip_parts[0].".".$ip_parts[1].".".$ip_parts[2];
			$iplink = "<a target=_blank href=detect_ip_mates.php?konzernid=$konzernid&ip=$ip123.%><font size=2><font color=blue>$ip123</font></font></a>";
			$iplink .= "<a target=_blank href=detect_ip_mates.php?konzernid=$konzernid&ip=$vl><font size=2><font color=black>.".$ip_parts[3]."</font></font></a>";
			$iplink .= "<br><font color=red>".$ipstrue[$kk]."</font>";
			...
			$ips_ausgabe .= "<tr bgcolor=white>
						...
						<td bgcolor=#f4f4f4><a href=detect_ip_mates.php?konzernid=$longest_online><font size=2 color=".($longest_online == $konzernid ? "green" : "red")."><b>".$other_playerdata[$longest_online][syndicate]." (#".$other_playerdata[$longest_online][rid].")</a></b> [".round($different_players_onlinetime_by_ip[$vl][$longest_online] / $time_ip_online[$vl] * 100)."%]</font></td>
						<td bgcolor=#f4f4f4>".($second_longest_online ? "<a href=detect_ip_mates.php?konzernid=$second_longest_online><font  size=2 color=".($second_longest_online == $konzernid ? "green" : "red")."><b>".$other_playerdata[$second_longest_online][syndicate]." (#".$other_playerdata[$second_longest_online][rid].")</a></b> [".round($different_players_onlinetime_by_ip[$vl][$second_longest_online] / $time_ip_online[$vl] * 100)."%]</font>":"")."</td>
		...
		foreach ($lastused_pcid as $vl => $last_used) {
			...
			$pcids_ausgabe .= "<tr bgcolor=white>
						...
						<td bgcolor=#fafc76><a href=detect_ip_mates.php?konzernid=$konzernid&pcid=$vl><font size=2><font color=black>".substr($vl, 0, 6).substr($vl, strlen($vl)-7, 8)."</font></a></font></td>
						<td bgcolor=#d6ddf9><font size=2>-<b>$number_of_players</b>-</font></td>
						<td bgcolor=#d6ddf9><font size=2>-$number_of_logins-</font></td>
						...
						<td bgcolor=#f4f4f4><a href=detect_ip_mates.php?konzernid=$longest_online><font size=2 color=".($longest_online == $konzernid ? "green" : "red")."><b>".$other_playerdata[$longest_online][syndicate]." (#".$other_playerdata[$longest_online][rid].")</a></b> [".round($different_players_onlinetime_by_pcid[$vl][$longest_online] / $time_pcid_online[$vl] * 100)."%]</font></td>
						<td bgcolor=#f4f4f4>".($second_longest_online ? "<a href=detect_ip_mates.php?konzernid=$second_longest_online><font  size=2 color=".($second_longest_online == $konzernid ? "green" : "red")."><b>".$other_playerdata[$second_longest_online][syndicate]." (#".$other_playerdata[$second_longest_online][rid].")</a></b> [".round($different_players_onlinetime_by_pcid[$vl][$second_longest_online] / $time_pcid_online[$vl] * 100)."%]</font>":"")."</td>
		...
		foreach ($total_sessions_by_ips[$ip] as $vl) {
			...
			if (++$count >= 2 && $count <= count($total_sessions_by_ips[$ip])) {
				...
				if ($zwischentime > 10 * 60) {
					$sessions_ausgabe .= "<tr bgcolor=white".($zwischentime > 0 ? " height=".(round($zwischentime/60/10)*10 < 360 ? round($zwischentime/60/10)*10 : 360):"")."><td colspan=9 align=center valign=center bgcolor=".($zwischentime < 0 ? "#fffaad":"white")."><font size=".($zwischentime > 0 ? ceil($zwischentime/3600/6) : "1").">".($zwischentime >= 86400 ? floor($zwischentime/86400)."d, ":"").($zwischentime >= 3600 ? (floor($zwischentime/3600) % 24)."h, ":"").(floor($zwischentime/60) % 60)."m, ".($zwischentime % 60)."s</font></td></tr>";
				}
			}
			...
			$sessions_ausgabe .= "<tr bgcolor=white>".($newday ? "
							...
							<td align=right bgcolor=#c3dbc0><font size=2><a href=detect_ip_mates.php?konzernid=$konzernid&session_id=".$vl['sessionid'].">".round($vl[validtime]/60)."m</a></font></td>
							<td bgcolor=#c3dbc0><font size=2>".date("H:i:s",$vl[gueltig_bis])."</font></td>
							<td bgcolor=#f4f4f4><font size=2><a href=detect_ip_mates.php?konzernid=$vl[user_id]>".($vl[user_id] == $konzernid ? "<font color=blue><b>":"<font color=black>").$other_playerdata[$vl[user_id]][syndicate]." (#".$other_playerdata[$vl[user_id]][rid].")".($vl[user_id] == $konzernid ? "</b></font>":"</font>")."</a></font></td>
							<td><font size=2><a href=detect_ip_mates.php?konzernid=$vl[user_id]&pcid=$vl[pc_identifier]>".substr($vl[pc_identifier], 0, 6).substr($vl[pc_identifier], strlen($vl[pc_identifier])-7, 8)."</a></font></td>
							".(preg_match("/%/", $ip) ? "
							<td><font size=2><a href=detect_ip_mates.php?konzernid=$vl[user_id]&ip=$vl[ip] target=_blank>".($vl['ip'])."</a></font></td>":"")."
							<!--
	...
	elseif($pcid) {
		...
		foreach ($total_sessions_by_pcid[$pcid] as $vl) {
			...
			$sessions_ausgabe .= "<tr bgcolor=white>".($newday ? "
							...
							<td bgcolor=#c3dbc0><font size=2>".date("H:i:s",$vl[gueltig_bis])."</font></td>
							<td bgcolor=#f4f4f4><font size=2><a href=detect_ip_mates.php?konzernid=$vl[user_id]>".($vl[user_id] == $konzernid ? "<font color=blue><b>":"<font color=black>").$other_playerdata[$vl[user_id]][syndicate]." (#".$other_playerdata[$vl[user_id]][rid].")".($vl[user_id] == $konzernid ? "</b></font>":"</font>")."</a></font></td>
							<td><font size=2><a href=detect_ip_mates.php?konzernid=$vl[user_id]&ip=$vl[ip]>".$vl[ip]."</a></font></td>
							<!--
							<td><font size=2>".preg_replace("/\([^\)]*\)/", "", $vl[browsername])."</font></td>
							<td><font size=2>$vl[hostname]</font></td>-->
							</tr>";
	...
	// Andernfalls wenn Sessionid übergeben wurde, die Klicks dazu rausfinden

	elseif($session_id) {

		$pages = assocs("select * from pages", "id");

		$session = assoc("select * from sessionids_safe where sessionid='".addslashes($session_id)."' and angelegt_bei >= ".($globals[roundstarttime]-3*86400));
		
		if ($session) {
			$userdaten_session = assoc("select * from users where konzernid = ".$session['user_id']);
			if (!$userdaten_session) {
				echo "Der User hat den Konzern gelöscht.";
			} else {
				echo "Die Sessionid $session_id gehört zum User \"".$userdaten_session['username']."\" mit der Userid ".$userdaten_session['id']."<br><br>";
			}
			echo "Browsername: ".$session['browsername']."<br>Hostname: ".$session['hostname']."<br><br>";
			echo "Gültigkeitsdauer der Session: <br>START:&nbsp;".date("H:i:s, d.M", $session['angelegt_bei'])."<br>ENDE:&nbsp;&nbsp;&nbsp;".date("H:i:s, d.M", $session['gueltig_bis'])."<br><br>";
			$clicks = assocs("select * from heaptable2 where clicktime >= ".$session['angelegt_bei']." and clicktime <= ".$session['gueltig_bis']."+2 and user_id = ".$session['user_id']." order by clicktime asc");
			echo "<table align=center width=400>";
			foreach ($clicks as $vl) {
				echo "<tr><td>".date("H:i:s", $vl['clicktime'])."</td><td>".($pages[$vl['seite']]['name'] ? $pages[$vl['seite']]['name'] : $pages[$vl['seite']]['dateiname'])."</td></tr>";
			}
			echo "</table>";
		}
	}
...
function move_old_sessionids_actual_to_safe_table($id) {
  $time = time();
  // Nachschauen ob noch Sessionid im Actual table vorhanden und in safe schreiben
  $result= rows("select sessionid, angelegt_bei, gueltig_bis, ip, user_id, pc_identifier, browsername, hostname from sessionids_actual where user_id='$id' and gueltig_bis < $time");
  if (count($result) > 0) {
    select ("delete from sessionids_actual where user_id='$id'");
	  foreach ($result as $ky => $vl)	{
		  ...
		  select ("insert into sessionids_safe (sessionid, angelegt_bei, gueltig_bis, ip, user_id, pc_identifier, browsername, hostname) values ('$vl[0]',$vl[1],$endtime,'$vl[3]', $vl[4], '$vl[5]', '$vl[6]', '$vl[7]')");
	  }
  }
}

<?
	...
	if ($inneraction == 1) {
		$id = floor($id);
		$attackdata_own = assocs("select type, did, drid, drace, time, winner, apoints, dpoints, landgain from attacklogs where aid='$id' and ginactive=0 and warattack=0");
		$exceptions = "Angriffe von $id außer Angriffe auf Inaktive und Kriegsangriffe<br>";
		if ($attackdata_own) {
			...
			foreach ($times as $ky => $vl) {
				...
				if ($timesdata[$ky][defense]) {	#Spydata
					$color = "lightgreen";
					if ($timesdata[$ky][aid] != $id): $color = "green"; endif;
					$lines[1] = $names[$timesdata[$ky][did]][syndicate]." (#".$names[$timesdata[$ky][did]][rid].") [".$names[$timesdata[$ky][aid]][race]."]";
					$lines[2] = $timesdata[$ky][aid] == $id ? "":$names[$timesdata[$ky][aid]][syndicate]." (#".$names[$timesdata[$ky][aid]][rid].") [".$names[$timesdata[$ky][aid]][race]."]";
		...
		else { $ausgabe .= "$id hat keine Angriffe getätigt."; }

	}
	else {
		$ausgabe .= "<form action=$self><input type=hidden name=action value=checkuser><input type=hidden name=inneraction value=1>Konzernid: <input type=text name=id><br><br><input type=submit value=go></form>";
	}

...
if ($inner) {
	echo "<a href=".$self."?action=adminstats>Back</a><br>";
}
...
if ($inner == "units") {
	...
	$ausgabe.="<br><b><i> Einheiten gesamt: ".pointit($sum)."</i></b><br><br><table width=300 align=center>";

	foreach ($all as $key => $value) {
		$ausgabe.="<tr><td>$key</td><td align=right> ".pointit($value)."</td><td align=right>".(round($value / $sum * 10 * 100)/10)."</td></tr>";
	}
	$ausgabe.="</table><br><br>Nach Fraktionen pro Spieler:<br><br><table width=200 align=center>";
	foreach ($units as $trace => $tunits) {
		foreach ($tunits as $name => $number) {
			$ausgabe.="<tr><td>".$unitsettings[$trace][$name][name]."</td><td align=right>".pointit(round($number/$rnumber[$trace]))."</td></tr>";
		}
		$ausgabe.="<tr><td colspan=2><hr></td></tr>";
	}
}
if (!$inner) {
	echo "
